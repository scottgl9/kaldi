...
for part in dev-clean test-clean dev-other test-other train-clean-100; do
  ...
  if [ ! -d data/$(echo $part | sed s/-/_/g) ]; then
...
if [ ! -d data/lang_nosp_test_fglarge ]; then
...
if [ ! -d exp/mono_ali_5k ]; then
  # decode using the monophone model
  (
    if [ ! -d exp/mono/graph_nosp_tgsmall ]; then
      utils/mkgraph.sh data/lang_nosp_test_tgsmall \
        exp/mono exp/mono/graph_nosp_tgsmall
    fi
    for test in test_clean test_other dev_clean dev_other; do
      if [ ! -d exp/mono/decode_nosp_tgsmall_$test ]; then
...
if [ ! -d exp/tri3b_ali_clean_100 ]; then
...
if [ ! -f exp/tri4b/pron_bigram_counts_nowb.txt ]; then
...
if [ ! -d exp/tri4b_ali_clean_100 ]; then
